...
CustomTabsInkBar::CustomTabsInkBar(CustomTabs *parent)
...
CustomTabsInkBar::~CustomTabsInkBar()
{
}

void CustomTabsInkBar::refreshGeometry()
{
...
bool CustomTabsInkBar::eventFilter(QObject *obj, QEvent *event)
{
	switch (event->type())
	{
	case QEvent::Move:
	...
	return CustomOverlayWidget::eventFilter(obj, event);
}

void CustomTabsInkBar::paintEvent(QPaintEvent *event)
{
	Q_UNUSED(event)
...
CustomTab::CustomTab(CustomTabs *parent)
	: CustomFlatButton(parent),
	m_tabs(parent),
	m_active(false)
{
	Q_ASSERT(parent);
...
QSize CustomTab::sizeHint() const
{
	if (icon().isNull()) {
		return CustomFlatButton::sizeHint();
	}
	else {
...
void CustomTab::activateTab()
{
	m_tabs->setCurrentTab(this);
}

void CustomTab::paintForeground(QPainter *painter)
{
