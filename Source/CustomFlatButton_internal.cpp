...
CustomFlatButtonStateMachine::CustomFlatButtonStateMachine(CustomFlatButton *parent)
	...
	m_wasChecked(false)
{
	...
	transition = new CustomStateTransition(FlatButtonCheckedTransition);
	...
	transition = new CustomStateTransition(FlatButtonUncheckedTransition);
	...
	transition = new CustomStateTransition(FlatButtonPressedTransition);
...
CustomFlatButtonStateMachine::~CustomFlatButtonStateMachine()
{
}

void CustomFlatButtonStateMachine::setOverlayOpacity(qreal opacity)
{
	m_overlayOpacity = opacity;
	m_button->update();
}

void CustomFlatButtonStateMachine::setCheckedOverlayProgress(qreal progress)
{
	m_checkedOverlayProgress = progress;
	m_button->update();
}

void CustomFlatButtonStateMachine::setHaloOpacity(qreal opacity)
{
	m_haloOpacity = opacity;
	m_button->update();
}

void CustomFlatButtonStateMachine::setHaloSize(qreal size)
{
	m_haloSize = size;
	m_button->update();
}

void CustomFlatButtonStateMachine::setHaloScaleFactor(qreal factor)
{
	m_haloScaleFactor = factor;
	m_button->update();
}

void CustomFlatButtonStateMachine::startAnimations()
{
	m_haloAnimation->start();
	start();
}

void CustomFlatButtonStateMachine::setupProperties()
{
	QColor overlayColor;
...
void CustomFlatButtonStateMachine::updateCheckedStatus()
{
	const bool checked = m_button->isChecked();
	if (m_wasChecked != checked) {
		m_wasChecked = checked;
		if (checked) {
			postEvent(new CustomStateTransitionEvent(FlatButtonCheckedTransition));
		}
		else {
			postEvent(new CustomStateTransitionEvent(FlatButtonUncheckedTransition));
		}
	}
}

bool CustomFlatButtonStateMachine::eventFilter(QObject *watched,
	QEvent  *event)
{
	//customfocus
	if (QEvent::FocusIn == event->type()) {
		...
		if (focusEvent && Qt::MouseFocusReason == focusEvent->reason()) {
			//m_button->setForegroundColor(Qt::red); // fab share
			m_button->isFocus = true;
			postEvent(new CustomStateTransitionEvent(FlatButtonPressedTransition));
			return true;
		}
	}
	else if (QEvent::FocusOut == event->type())
	{
		//m_button->setForegroundColor(Qt::black);
		
		//focus maintain
		if(m_button->autoFocus)
			m_button->isFocus = false;
	}

	return QStateMachine::eventFilter(watched, event);
}

void CustomFlatButtonStateMachine::addTransition(QObject *object,
...
void CustomFlatButtonStateMachine::addTransition(QAbstractTransition *transition,
	QState *fromState,
	QState *toState)
{
